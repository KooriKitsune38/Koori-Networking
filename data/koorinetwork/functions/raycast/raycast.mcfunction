# Add 1 to score Ray #
scoreboard players add @s NetworkingRay 1
...
execute if entity @s[tag=RegisteredToNetwork] unless block ~ ~ ~ #signs{Text2:'{"color":"#FFADD1","text":"New User"}'} unless block ~ ~ ~ #signs{Text2:'{"text":"New User","color":"#FFADD1"}'} run scoreboard players reset @s RegisterNetwork

# Repeat Ray #
execute if score @s NetworkingRay matches ..10 positioned ^ ^ ^.5 if block ~ ~ ~ #koorinetwork:raycast_exceptions run function koorinetwork:raycast/raycast